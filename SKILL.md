...
**One-shot alternative**: `install.sh --with-plugins` (opt-in flag, or `curl … | bash -s -- --with-plugins`) does the marketplace-add + install + enable for the 4 plugins at install time, with the same anti-clobber re-enable — so a fresh machine gets the plugins (incl. ponytail) without a separate `activate`. If the `rtk` binary is already present it also runs `rtk init -g` to wire RTK's hook; the rtk binary itself is a Rust build, installed separately.
